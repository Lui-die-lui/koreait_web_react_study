...
function InputState4() {
  ...
  const onClickHandler = () => {
    // setProducts((prev) => {
    //   const newArray = prev; -> 원본임
    //   newArray.push(inputValue);
    //   return newArray;
    // });
    // push는 원본 배열을 직접 수정하기 때문에 리액트의 불변성을 위배
    // 리액트는 상태의 주소값이 바뀌어야 변화를 감지하는데 push는 주소값을 바꾸지 않기때문에
    // 재렌더링이 일어나지 않거나 예기치 않은 버그가 발생할 수 있다

    setProducts((prev) => [...prev, inputValue]); // 원본 배열을 복사해와서 새로 set
      ...
      <table>
        ...
        <tbody>
          ...
          {products.map((product, index) => (
            // 리액트에서 목록을 렌더링 할 때는 각 항목을 구분할 수 있는 고유한 key를 반드시 제공해야한다
            // key는 리액트가 어떤 항목이 변경, 추가, 삭제 되었는지 효율적으로 파악하는 데 사용된다
