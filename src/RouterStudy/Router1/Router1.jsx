...
function Router1() {
  ...
  // 특정 로직 ex) 로그인 성공 시 메인페이지로 전환

  /**
   * React Router Dom
   * 리액트 => Single Page Application (SPA)
   * 페이지 이동(라우팅) 을 구현할 수 있게 해주는 라이브러리
   * 페이지의 주소나 링크를 클릭했을때 페이지 전체를 새로 고침하지 않고 주소에 맞는 특정 컴포넌트만
  ...
  return (
    <div css={layout}>
      ...
      {/* Routes는 여러 Route들을 감싸는 영역 */}
      {/* Route는 특정 경로(path)와 그 경로에서 보여줄 컴포넌트(element)를 이어주는 역할 */}
      {/* 브라우저의 주소창이 path와 일치하는 Route를 찾아 해당 컴포넌트를 화면에 띄워줌 */}
      {/* 단 Routes안에 있는 여러 Route중 path가 일치하는것 하나만 띄워줌 */}
