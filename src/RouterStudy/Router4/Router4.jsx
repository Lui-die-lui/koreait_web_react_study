...
// myPage, 로그인 후 상세페이지 -> useParams
// 해당 카테고리 게시물 검색 -> searchParams
function Component1() {
  // 비구조 할당으로 가져옴
  ...
  //  /param1/lsg 입력 시 path parameter는 lsg입니다. 값 받아볼 수 있음.
}

function Component2() {
  // useSearchParams 현재 search params, params를 변경하는 함수 형태의 배열의 반환
  const [searchParams, setSearchParams] = useSearchParams();

  //   http://localhost:5173/param2/?name=lsg
  // 만약 다 가져오려면
  useEffect(() => {
    ...
    // .getAll() 으로 같은 이름의 모든 값을 가져올 수 있다.
...
function Router4() {
  return (
    <div>
      <Routes>
        {/* path에 콜론을 사용하면 해당 위치의 URL 일부를 name 이라는 변수로 받겠다라는 의미 */}
